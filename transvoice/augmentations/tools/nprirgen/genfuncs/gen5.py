...
# Generate RIRs. 
def genrir5(soundVelocity, fs, receiverPositions, sourcePosition, roomMeasures, betaCoeffs, orientation, isHighPassFilter, nOrder, nSamples, micType):
    if nOrder != -1:
    ...
    for idxMicrophone in range(nMicrophones):
        ...
        gain = np.where(fdist < nSamples, gain, 0)
            ...
            for my in range(2*n+1):
                for mz in range(2*n+1):
                    for q in range(0, 2):
                        for j in range(0, 2):
                            for k in range(0, 2):
                                if src_e[mx, q, my, j, mz, k] - src_b[mx, q, my, j, mz, k] > 0:
                                    imp[idxMicrophone, dest_b[mx, q, my, j, mz, k] : dest_e[mx, q, my, j, mz, k]] += gain[mx, q, my, j, mz, k] * LPI[src_b[mx, q, my, j, mz, k] : src_e[mx, q, my, j, mz, k], mx, q, my, j, mz, k]
