...
import numpy as np
import glob
from tools.nprirgen import nprirgen
...
class dataset:
    ...
    def fetch_signals(self, number_of_speakers):
        ...
        for i in range(self.num_of_speakers):
            ...
            select_name_indx = np.random.randint(low=0, high=len(cand_names), size=1)
    ...
    def position_valid(self, room_dims, speaker_pos):
        if (
            speaker_pos[0] > 0.5
            and speaker_pos[0] < room_dims[0] - 0.5
            and speaker_pos[1] > 0.5
            and speaker_pos[1] < room_dims[1] - 0.5
        ):
            return 1
        else:
            return 0
    ...
    def room_gen(self, room_dims, RT60):
        ...
        for i in range(self.num_of_speakers):
            ...
            while not invalid:
                ...
                speaker_distance = 1.5 + round(np.random.uniform(low=-0.2, high=0.2), 2)
    ...
    def gen_scene(self, scenario_num_of_speakers, scene_i, current_write_path):
        ...
        noise_gain = np.sqrt(
            10 ** (-snr / 10) * np.std(Mixed) ** 2 / np.std(noise) ** 2
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in_path", type=str, default="dataset/2speakers/wav8k/min/tr/s1", help=""
    )
    parser.add_argument("--out_path", type=str, default="dataset/tmp", help="")
    parser.add_argument(
        "--noise_path", type=str, default="dataset/wham_noise/tr", help=""
    )
    parser.add_argument(
        "--num_of_speakers", type=int, default=2, help="no of speakers."
    )
