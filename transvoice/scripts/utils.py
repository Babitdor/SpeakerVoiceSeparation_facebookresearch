...
class LogProgress:
    ...
    def _log(self):
        elapsed = time.time() - self._begin
        if elapsed < 1e-6:
            elapsed = 1e-6  # Prevent division by zero
        self._speed = (1 + self._index) / elapsed
    ...
    for p in model.parameters():
        ...
        if is_first:
            ...
            total_norm = torch.cat(
                (total_norm.unsqueeze(1), p.data.grad.flatten().unsqueeze(1)), dim=0
            ).squeeze(1)
    return total_norm.norm(2) ** (1.0 / 2)
...
def calculate_weight_norm(model):
    ...
    for p in model.parameters():
        ...
        if is_first:
            ...
            total_norm = torch.cat(
                (total_norm.unsqueeze(1), p.data.flatten().unsqueeze(1)), dim=0
            ).squeeze(1)
    return total_norm.norm(2) ** (1.0 / 2)
...
def overlap_and_add(signal, frame_step):
    ...
    ones = signal.new_ones(*outer_dimensions, frames, frame_length)
    overlap_count = signal.new_zeros(*outer_dimensions, output_size)
    overlap_count = overlap_count.view(
        *outer_dimensions, output_subframes, subframe_length
    )
    overlap_count.index_add_(
        -2, frame, ones.view(*outer_dimensions, -1, subframe_length)
    )
    overlap_count = overlap_count.view(*outer_dimensions, -1)
    # Avoid division by zero
    overlap_count = torch.clamp(overlap_count, min=1.0)
    result = result / overlap_count
