...
from .data.data import Validset, EvalDataLoader
from . import distrib
from .scripts.utils import bold, deserialize_model, LogProgress


logger = logging.getLogger(__name__)

parser = argparse.ArgumentParser("Evaluate separation performance using MulCat blocks")
parser.add_argument("model_path", help="Path to model file created by training")
parser.add_argument(
    "data_dir", help="directory including mix.json, s1.json, s2.json, ... files"
)
...
def evaluate(args, model=None, data_loader=None, sr=None):
    ...
    # Load model
    if not model:
        pkg = torch.load(args.model_path, map_location=args.device, weights_only=False)
        if "model" in pkg:
            model = pkg["model"]
        else:
            model = pkg
    ...
    # Load data
    if not data_loader:
        dataset = Validset(args.data_dir, pad=True, subset_size=args.val_subset_size)
    ...
    with ProcessPoolExecutor(args.num_workers) as pool:
        with torch.no_grad():
            iterator = tqdm.tqdm(data_loader, desc="Eval estimates", unit="batch")
            for i, data in enumerate(iterator):
                ...
                pendings.append(
                    pool.submit(
                        _run_metrics, sources, reorder_estimate, mixture, None, sr=sr
                    )
                )
                total_cnt += sources.shape[0]

            for pending in tqdm.tqdm(pendings, desc="Eval metrics", unit="batch"):
    ...
    sisnr, pesq, stoi = distrib.average([m / total_cnt for m in metrics], total_cnt)
    logger.info(
...
def cal_SISNR(ref_sig, out_sig, eps=1e-8):
    ...
    proj = (np.sum(ref_sig * out_sig, axis=1).reshape(B, 1)) * ref_sig / ref_energy
    noise = out_sig - proj
...
def main():
    ...
    json.dump({"sisnr": sisnr, "pesq": pesq, "stoi": stoi}, sys.stdout)
    sys.stdout.write("\n")
