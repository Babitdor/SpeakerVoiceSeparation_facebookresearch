...
class Solver(object):
    def __init__(self, data, model, optimizer, args, scaler):
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            ...
            # print(
            #     f"[DEBUG] mixture shape: {mixture.shape}, lengths shape: {lengths.shape}, sources shape: {sources.shape}"
            # )
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                if not cross_valid:
                    print(
                        ...
                        estimate_source.min().item(),
                        estimate_source.max().item(),
                        estimate_source.mean().item(),
                    )
                    print(
                        "sources stats:",
                        sources.min().item(),
                    ...
                    if torch.isnan(estimate_source).any():
                        print("NaN detected in estimate_source!")
                    if torch.isnan(sources).any():
                        print("NaN detected in sources!")
                    loss.backward()
