...
from . import distrib
from .scripts.utils import remove_pad, bold, deserialize_model, LogProgress
...
parser.add_argument(
    "--mix_dir", type=str, default=None, help="Directory including mix wav files"
)
parser.add_argument(
    "--mix_json", type=str, default=None, help="Json file including mix wav files"
)
...
def save_wavs(estimate_source, mix_sig, lengths, filenames, out_dir, sr=16000):
    ...
    for i, filename in enumerate(filenames):
        filename = os.path.join(out_dir, os.path.basename(filename).strip(".wav"))
...
def write(inputs, filename, sr=8000):
    # librosa.write_wav(filename, inputs, sr, norm=True)
...
def separate(args, model=None, local_out_dir=None):
    ...
    if not mix_json and not mix_dir:
        logger.error(
            "Must provide mix_dir or mix_json! "
    ...
    eval_loader = distrib.loader(eval_dataset, batch_size=1, klass=EvalDataLoader)
