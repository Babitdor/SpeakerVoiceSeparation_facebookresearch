...
# Generate RIRs. 
def genrir2(soundVelocity, fs, receiverPositions, sourcePosition, roomMeasures, betaCoeffs, orientation, isHighPassFilter, nOrder, nSamples, micType):
    if nOrder != -1:
                        ...
                        for j in range(0, 2):
                            for k in range(0, 2):
                                if fdist[mx, q, my, j, mz, k] < nSamples:
                                    ...
                                    b = max(max(startPositions[mx, q, my, j, mz, k], 0) - startPositions[mx, q, my, j, mz, k], 0)
                                    e = max(min(startPositions[mx, q, my, j, mz, k] + Tw, nSamples) - startPositions[mx, q, my, j, mz, k], 0)
                                    src = list(range(b, e))
                                    imp[idxMicrophone, src + startPositions[mx, q, my, j, mz, k]] += gain[mx, q, my, j, mz, k] * LPI[src, mx, q, my, j, mz, k]
